import React from 'react';
import Riser from '../layout/Riser'

const Success = () => {
  return (
    <div >
      <h1 className="center">Our Success Stories</h1>
      <div class="container">
      <div  className="row">
        <div className="col s3 m4">
          <div className="card">
            <div className="card-image">
              <img src="https://images.unsplash.com/photo-1517976487492-5750f3195933?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1350&q=80"></img>
            </div>
            <div className="card-content">
              <span className="card-title">
                <h5>Chicken Farming Success Story</h5>
              </span>
              <p className="truncate">
                I am a very simple card. I am good at containing small bits of information. I am convenient because I
                require little markup to use effectively.
              </p>
            </div>
            <div className="card-action">
              <button className="btn waves-effect waves-light blue darken-4">See More</button>
            </div>
          </div>
        </div>
        <div className="col s3 m4">
          <div className="card">
            <div className="card-image">
              <img src="https://images.unsplash.com/photo-1517976487492-5750f3195933?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1350&q=80"></img>
            </div>
            <div className="card-content">
              <span className="card-title">
                <h5>Chicken Farming Success Story</h5>
              </span>
              <p className="truncate">
                I am a very simple card. I am good at containing small bits of information. I am convenient because I
                require little markup to use effectively.
              </p>
            </div>
            <div className="card-action">
              <button className="btn waves-effect waves-light blue darken-4">See More</button>
            </div>
          </div>
        </div>
        <div className="col s3 m4">
          <div className="card">
            <div className="card-image">
              <img src="https://images.unsplash.com/photo-1517976487492-5750f3195933?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1350&q=80"></img>
            </div>
            <div className="card-content">
              <span className="card-title">
                <h5>Chicken Farming Success Story</h5>
              </span>
              <p className="truncate">
                I am a very simple card. I am good at containing small bits of information. I am convenient because I
                require little markup to use effectively.
              </p>
            </div>
            <div className="card-action">
              <button className="btn waves-effect waves-light blue darken-4">See More</button>
            </div>
          </div>
        </div>
      </div>
      </div>
      <div className="card center">
        <div className="col s12 m7">
          <div className="card">
            <div className="card-content ">
              <i className="material-icons ">mail</i>
              <span className="card-title">
                <h5>Subscribe To Our Newsletter</h5>
              </span>
              <p>
                I am a very simple card. I am good at containing small bits of information. I am convenient because I
                require little markup to use effectively.
              </p>
              
            </div>
          </div>
        </div>
      </div>
      <footer className="page-footer grey darken-2">
          <div className="container">
            <div className="row">
              <div className="col 4 s3">
                <h5 className="white-text">Footer Content</h5>
                {/* <a className="white-text"><Riser/></a> */}
                <p className="grey-text text-lighten-4">You can use rows and columns here to organize your footer content.</p>
              </div>
              <div className="col 4 s3">
                <h5 className="white-text">Resources</h5>
                <ul>
                  <li><a className="grey-text text-lighten-3" href="#!">Learning Center</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">OKRs for stattups</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">Downloads</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">Support</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">Security policy</a></li>
                </ul>
              </div>
              <div className="col 4 s3">
                <h5 className="white-text">Company</h5>
                <ul>
                  <li><a className="grey-text text-lighten-3" href="#!">About</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">Projects</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">Downloads</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">Terms of Service</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">Privacy policy</a></li>
                </ul>
              </div>
              <div className="col 4 s3">
                <h5 className="white-text">Contact Details</h5>
                <ul>
                  <li><a className="grey-text text-lighten-3" href="#!">Call: 072500000</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">073500000</a></li>
                  <li><a className="grey-text text-lighten-3" href="#!">info@riserafrica254.com</a></li>
                  
                </ul>
              </div>
              <br />
              <div className="row">
              <a className="white-text"><Riser/></a>
                </div>
            </div>
          </div>
          <div className="footer-copyright">
            <div className="container">
            © 2020 Copyright Text
            <a className="grey-text text-lighten-4 right" href="#!">More Links</a>
            </div>
          </div>
        </footer>
    </div>
  );
};

export default Success;
